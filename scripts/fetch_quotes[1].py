...
def sanitize(obj):
    ...
    Python's json module allows NaN by default and writes it as a bare
    `NaN` token - that is NOT valid JSON, and browsers' JSON.parse()
    correctly rejects it (this is exactly what broke the live page: a
    ticker with a data gap let a NaN price flow straight into the output).
    Called once on the whole snapshot right before writing, so it's a
    single backstop no matter where a stray NaN enters.
    """
...
def fetch_one(company: dict, with_history: bool = False):
    ...
    try:
        ...
        # Drop any rows with a missing Close - Yahoo occasionally returns a
        # gap day, and letting a NaN Close flow into price/return math is
        # exactly what produced the invalid-JSON bug (a literal NaN token).
...
def main() -> int:
    ...
    # allow_nan=False is a hard backstop: if a NaN/Infinity ever slipped past
    # sanitize() somehow, this makes the workflow fail loudly in the Actions
    # log instead of silently committing invalid JSON that breaks the page.
